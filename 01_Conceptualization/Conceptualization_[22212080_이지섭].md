...
## Use Case List
* 빈도별 정렬
  * 사용자가 주로 조회하는 OTP를 파악하여 앞부분에 뜰 수 있도록 정렬한다

* 상단고정(북마크)
  * 사용자가 특별히 원하는 OTP에 대해 상단에 고정하여 편리성을 증대한다

* 카테고리화
  * 동일 혹은 유사 서비스의 OTP는 추가시 자동 감지 혹은 사용자의 지정으로 동일한 카테고리에 묶어 보기 편하게 한다

* 지문인식 및 PIN
  * 제3자의 부정접근을 방지하기 위해 OTP를 보기 위해서는 지문 인식을 요구하며, 사용할 수 없는 경우에는 초기에 설정한 PIN을 요구한다.

* 코드백업
  * 유사시에 대비하여 사용자의 OTP 전체를 패스워드로 암호화 하여 파일로 내보내기하여 백업할 수 있도록 한다.
