...
| | 3a.1 다시 사용자에게 인증을 요구한다 |
| | 3a.2 인증이 성공한 경우에만 내보내기 절차를 계속한다 (Use case #08-4) |
...
| **3** | **3a 앱의 잠금 상태가 기준을 충족하지 못하는 경우** |
| | 3a.1 다시 사용자에게 인증을 요구한다 |
| | 3a.2 인증이 성공한 경우에만 가져오기 절차를 계속한다 (Use case #09-4) |
...
| **OTPRepository** | 기기 내 로컬 데이터베이스에 OTP 및 카테고리 데이터를 추가, 조회, 수정, 삭제하는 데이터 접근 계층을 담당하는 클래스이다. |
...
| **BackupManager** | 사용자의 전체 OTP 데이터를 암호화된 파일 형식으로 기기에 내보내거나, 비밀번호를 통해 기존 백업 파일에서 데이터를 읽어와 복원하는 기능을 수행한다. |
...
#### 4.1.1 스플래시
* 앱 실행 시 가장 먼저 표시되는 초기 화면으로, 화면 중앙에 'AuthCAT' 로고를 표시한다
* 화면 하단에 '시스템 생체인식 팝업'을 띄워 지문을 통한 빠흔 잠금 해제를 지원한다
* 생체 인식 대신 비밀번호(PIN)를 사용하고자 할 경우, 중앙의 'PIN 사용' 버튼을 탭하여 인증 방식을 전환할 수 있다
* 사용자 인증이 성공적으로 완료되면 메인 화면으로 이동한다

#### 4.1.2 메인
* 등록된 OTP 목록을 조회하고 관리하는 시스템의 핵심 화면이다
* 상단 영역에는 앱 타이틀과 함께 우측에 설정 및 카테고리 관리 버튼을 배치한다
* 카테고리 탭을 제공하여, 사용자가 원하는 카테고리의 OTP만 필터링하여 확인할 수 있도록 한다
* 각 OTP 항목은 서비스 이름, 계정 정보, 6자리 OTP 코드, 남은 시간을 나타내는 원형 타이머로 구성된다
* 특정 OTP 항목을 길게 누르면 해당 OTP의 정보를 수정할 수 있는 화면으로 이동한다
* 화면 우측 하단의 '+' 플로팅 액션 버튼을 누르면, OTP 추가를 위한 '직접 입력'과 'QR 스캔' 서브 메뉴가 나타난다

#### 4.1.3 OTP 추가/수정
* 새로운 OTP를 시스템에 등록하거나 기존 OTP 정보를 수정하는 폼 형식의 화면이다
* 사용자는 서비스 이름, 계정/이름, 카테고리(드롭다운 메뉴), 그리고 OTP 발급을 위한 시크릿 키를 입력한다
* 모든 정보를 입력한 후 화면 하단의 '추가' 또는 '수정' 버튼을 눌러 변경 사항을 시스템에 저장한다
* QR로 스캔하여 넘어온 경우에 시크릿 값은 자동으로 입력된다

#### 4.1.4 QR 스캐너
* 기기의 카메라를 이용하여 화면 중앙에 QR 코드를 인식할 수 있는 영역을 표시한다
* QR 코드를 성공적으로 스캔하면, 인식된 정보를 바탕으로 일부 필드가 채워진 'OTP 추가/수정' 화면(4.1.3)으로 전환된다

#### 4.1.5 카테고리 관리
* 메인 화면 우측 상단의 폴더 아이콘을 눌러 진입하는 카테고리 관리 화면이다
* 현재 생성된 카테고리 목록과 각 카테고리에 할당된 OTP 항목의 개수를 함께 표시한다
* 상단의 '+ 카테고리 추가' 버튼을 클릭하여 시스템에 새로운 카테고리를 생성한다
* 각 목록 좌측 드래그 핸들을 통해 카테고리의 노출 순서를 변경하거나, 이름을 수정, 혹은 삭제할 수 있다

#### 4.1.6 설정
* 메인 화면 우측 상단의 톱니바퀴 아이콘을 눌러 진입한다
* 'PIN 설정/변경', 'OTP 내보내기', 'OTP 불러오기'의 세 가지 주요 관리 메뉴를 리스트 형태로 제공한다
* 각 항목을 선택하면 해당하는 세부 설정 화면이나 팝업으로 이동한다

#### 4.1.7 보안 설정
* 앱 진입 시 사용할 PIN 번호를 설정 및 변경하고, 생체 인식 사용 여부를 관리하는 화면이다
* 보안을 위해 '현재 PIN'을 먼저 입력받은 후, '신규 PIN'과 '신규 PIN 확인' 필드를 통해 암호를 변경한다
* '지문 인식 사용' 체크박스를 제공하여 기기의 생체 인식 기능 사용 여부를 선택한다
* 설정 완료 후 하단의 '저장' 버튼을 눌러 적용한다

#### 4.1.8 OTP 내보내기
* 현재 기기에 저장된 모든 OTP 및 카테고리 데이터를 백업 파일로 추출하는 화면이다
* 생성될 백업 파일을 보호하기 위한 '암호' 및 '암호 확인' 필드를 제공한다
* 화면 하단에 내보낸 파일은 사용자가 입력한 암호로 암호화되며, 암호를 분실할 경우 데이터를 복구할 수 없다는 시스템 경고를 명시한다
* '내보내기' 버튼을 눌러 암호화 프로세스를 수행하고 백업 파일을 생성한다

#### 4.1.9 OTP 불러오기
* 선택 시 먼저 파일 선택기 화면을 띄워 불러올 파일을 선택받는다
* 백업 파일에 저장된 OTP 정보를 시스템에 복원하기 전, 사용자에게 최종 확인을 요구하는 팝업 화면이다
* 각종 안내 문구 표시
* 사용자는 '복원' 버튼을 눌러 OTP를 불러올 수 있다
