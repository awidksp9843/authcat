...
#### OTP
단일 OTP 항목의 데이터를 표현하는 클래스이다.

| 유형 | 이름 | 타입 | 설명 |
| --- | --- | --- | --- |
| **Attribute** | `- id` | `String` | 해당 OTP 데이터의 고유 식별자 |
| **Attribute** | `- serviceName` | `String` | 서비스 제공자 이름 |
...
| **Attribute** | `- usageCount` | `int` | 정렬 기준이 되는 사용 빈도수 |
...
#### OTPRepository
기기 내 로컬 데이터베이스에 OTP 및 카테고리 데이터를 추가, 조회, 수정, 삭제하는 클래스이다.
...
#### MainActivity
등록된 OTP 목록을 실시간으로 조회하고 타이머와 함께 화면에 표시하는 메인 액티비티이다.
...
#### OTPManageActivity
새로운 OTP를 수기나 스캔을 통해 시스템에 등록하거나 기존 메타데이터를 수정하는 폼 UI 액티비티이다.
...
#### SettingsActivity
애플리케이션의 보안 설정(생체 인식, PIN) 및 데이터 백업/복원 기능을 관리하는 UI 액티비티이다.
